...
## Tests on statsitics
I'm interested in what an ANOVA would tell me about differences in RMSEP among scenarios

```{r}
control.pls.rmsep <- map_dbl(control.pls,~plsda_RMSEP(.x, X_vars = -group, Y_var = group, CV = 7))
control.pca.rmsep <- map_dbl(control.pcr, "RMSEP")
needle.pls.rmsep <- map_dbl(needle.pls,~plsda_RMSEP(.x, X_vars = -group, Y_var = group, CV = 7))
needle.pca.rmsep <- map_dbl(needle.pcr, "RMSEP")
t.test(needle.pca.rmsep, needle.pls.rmsep)
t.test(control.pls.rmsep, control.pca.rmsep)
```
