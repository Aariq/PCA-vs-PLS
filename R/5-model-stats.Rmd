...
---
# TODO:
- Fix RMSEP function so it deals with errors (in RMSEP.R)
- Figure out how many models really fail during the RMSEP (I think it's just three, but I should check)
- Re-output table and edit Numbers doc
...
# null.pcr %>%
#   map_dfr("mod.stats", .id = "dataset") %>%
#   mutate(RMSEP = map_dbl(null.pcr, ~safe_pca_lr_RMSEP(.x, X_vars = -group, Y_var = group, CV = 7))) %>%
#   summarize_if(is.numeric, list(~mean(., na.rm = TRUE), ~sd(., na.rm = TRUE)))
# #   
```
