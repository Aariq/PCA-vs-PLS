...
#force second axis so there is something to plot.
...
```{r}
herring.pcalr <- pca_lr(herring, -group, group, CV = 7)
...
Make a plot with all four scenarios
```{r}
four_scenarios <- 
  plot_grid(null.cor.p + fix_theme_cor,
            control.cor.p + fix_theme_cor,
            needle.cor.p + fix_theme_cor,
            herring.cor.p + fix_theme_cor,
            null.pca.p + fix_theme_score,
            control.pca.p + fix_theme_score,
            needle.pca.p + fix_theme_score,
            herring.pca.p + fix_theme_score,
            null.pls.p + fix_theme_score + labs(title = NULL),
            control.pls.p + fix_theme_score + labs(title = NULL),
            needle.pls.p + fix_theme_score + labs(title = NULL),
            herring.pls.p + fix_theme_score + labs(title = NULL),
            nrow = 3, ncol = 4, align = "v", axis = "l", labels = glue::glue("({letters[1:12]})"), label_x = 0.05) %>% 
  plot_grid(color.bar, align = "h", axis = "t", rel_widths = c(12,1)) 

four_scenarios %>%
  save_plot(here("out", "four-scenarios.png"), ., ncol = 4, nrow = 3, base_height = 2.8, base_asp = 1.2)
```
