...
2. Correlated variables are not predictive ("red herring")
    ...
    - 10 variables with cov = 0.5 (axis 1)
    - 10 variables with cov = 0.2 (axis 2)
...
3. Correlated variables are predictive ("control")
    ...
    - 10 variables with cov = 0.2 (but not discriminating) (axis 2)
...
4. Predictive variables are not correlated ("needle in a haystack")
    ...
    - 10 variables with cov = 0.5
    - 10 variables with cov = 0.2
...
herring <- 
  future_map(1:500,
             ~sim_cat(n_obs = 20, n_groups = 2) %>% 
               ...
               sim_covar(n_vars = 10, var = 1, cov = 0.2, name = "C2")
  ) %>% 
...
control <- 
  future_map(1:500,
             ~sim_cat(n_obs = 20, n_groups = 2) %>% 
               ...
               sim_discr(n_vars = 10, var = 1, cov = 0.2, group_means = c(-1, 1), name = "D2") %>% 
               ungroup()
  ) %>% 
