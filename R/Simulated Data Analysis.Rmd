...
null.pcas.failed <- map_lgl(null.pcas.try, is.null)
null.pcas <- null.pcas.try[!null.pcas.failed]

# map to all datasets in needle in a haystack scenario
needle.pcas.try <-
  map(needle_in_haystack,
      ~safe_opls(select(., -group), plotL = FALSE, printL = FALSE))
needle.pcas.failed <- map_lgl(needle.pcas.try, is.null)
needle.pcas <- needle.pcas.try[!needle.pcas.failed]

# map to all datasets in red herring scenario
herring.pcas.try <-
  map(red_herring,
      ~safe_opls(select(., -group), plotL = FALSE, printL = FALSE))
herring.pcas.failed <- map_lgl(herring.pcas.try, is.null)
herring.pcas <- herring.pcas.try[!herring.pcas.failed]
```
...
needle.cors <-
  map2(.x = needle.pcas,
       ...
       .y = needle_in_haystack[!needle.pcas.failed], 
