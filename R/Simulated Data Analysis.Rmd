...
# To ask Elizabeth:
Currently if the model fails (e.g. because the first component isn't significant) I'm just ignoring it.  Is it better to force 2 axes for all models instead of allowing the function to choose the best number of axes (which might be zero)?
...
safe_opls <- possibly(partial(opls, printL = FALSE, plotL = FALSE), otherwise = NULL)
```
...
mypal <- qualitative_hcl(2, "Harmonic")
demoplot(mypal, "bar")
protan(mypal) %>% demoplot("bar")
```
...
library(cowplot)
panels <-
  plot_grid(control.p + theme(legend.position = "none"),
