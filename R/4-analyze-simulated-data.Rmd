...
null.pls.force <-
  null %>% 
  future_map(~ safe_opls(select(., -group), .$group, predI = 5, permI = 500)) %>% 
  future_map("result") %>% 
  discard(is.null)
```
...
```{r}
future_map_dbl(control.pls, ~ getSummaryDF(.x)$pre) %>% mean()
```
...
427/500 error because there are no significantly predictive components.  PLS is not fooled by the red herring!

```{r}
#force one component
herring.pls.force <-
  future_map(herring,
      ~ safe_opls(select(., -group), .$group, predI = 3, permI = 500)) %>% 
  future_map("result") %>% 
  discard(is.null)
```
...
write_rds(herring.pls.force, here("data", "models", "herring-pls-force.rds"))
```