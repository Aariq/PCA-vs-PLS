...
conf_pls <- function(pls.model){
  ...
  #using same critera as PCA
  
  scores <- get_scores(pls.model)
  data <- pls.model@suppLs$xModelMN
  # ensure that scores$y1 is numeric
  if(is.character(scores$y1)){
    scores <- scores %>% mutate(y1 = as.numeric(as.factor(y1)))
    message("Converting categorical Y variable to numeric")
  }
...
conf_pca(control.pls[[1]])
get_scores(control.pls[[1]])
```
