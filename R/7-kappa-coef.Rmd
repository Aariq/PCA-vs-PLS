...
control.pca.k <-
  control.pcas %>%
  future_map(conf_pcr) %>%
  future_map_dfr(calc_kappa, .id = "dataset") %>% 
  rename(pca.k = "kappa")
```
...
control.k.plotdata <-
  ...
  mutate(Method = as.factor(Method) %>%
           fct_recode(`PCA-LR` = "pca.k", `PLS-DA` = "pls.k", `PLS-DA VIP` = "pls.VIP.k"))

control.p <-
  ggplot(control.k.plotdata) +
  geom_density(aes(x = `Cohen's Kappa`, fill = Method, linetype = Method),
               alpha = 0.5, trim = TRUE, adjust = 4) +
...
needle.k.plotdata <-
  ...
  gather(-dataset, key = "Method", value = "Cohen's Kappa") %>% 
  mutate(Method = as.factor(Method) %>% 
           fct_recode(`PCA-LR` = "pca.k", `PLS-DA` = "pls.k", `PLS-DA: VIP > 1` = "pls.VIP.k"))
...
fig2 <-
  plot_grid(panels, get_legend(control.p),
            rel_widths = c(1, 0.15))
fig2
save_plot(here("out", "kappa.png"), fig2, ncol = 2, base_asp = 1.2)
save_plot(here("out", "figure3.svg"), fig2, ncol = 2, base_asp = 1.2)
save_plot(here("out", "figure3.pdf"), fig2, ncol = 2, base_asp = 1.2)
```
