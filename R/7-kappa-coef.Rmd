...
- Do correlation tests only with PCs that are significant in linear model.
...
scores <- test$pca %>% get_scores() %>% select(pcs)
  data <- test$pca@suppLs$xModelMN
cor(scores, data)

map(as.data.frame(data), ~cor.test(., scores$p1))
```
...
needle.k.plotdata <-
  ...
  gather(-dataset, key = "Method", value = "Cohen's Kappa") %>% 
  mutate(Method = as.factor(Method) %>% 
           fct_recode(PCA = "pca.k", `PLS-DA` = "pls.k", `PCA-DA: VIP > 1` = "pls.VIP.k"))
...
```{r}
panels <-
  plot_grid(control.p + theme(legend.position = "none")+ coord_cartesian(xlim = c(0, 1)),
            needle.p + theme(legend.position = "none")+ coord_cartesian(xlim = c(0, 1)),
            nrow = 1,
            ncol = 2,
            labels = "AUTO")
fig2 <-
