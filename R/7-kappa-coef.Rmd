...
- Make PLS-DA and PCA use same criteria (distance cutoff)
- Make function work with whatever number of axes there are in the model.
...
`conf_df()` creates a confusion matrix for models produced by `opls()` by calculating correlation of variables to scores for up to two principal components. These correlations are mathematically related to loadings. Then, it creates a cutoff correlation which is equivalent to the $r$ you would get if $p = 0.05$ in a correlation test.  It then categorizes variables as discriminating or not based on this cutoff and their distance from 0 in "correlation space".
...
`calc_kappa()` takes the results of `conf_df()` or `conf_pls()` and calculates Cohen's Kappa based on a confusion matrix.

```{r}
# opls.model <- pca.model <- control.pcas[[3]]

conf_df <- function(opls.model){
  scores <- get_scores(opls.model)
  data <- opls.model@suppLs$xModelMN
  ncomp = opls.model@summaryDF$pre
  
  #if its a pls, get rid of the Y variable
  if(opls.model@typeC == "PLS-DA"){
    scores = scores[ , -2]
  }
  
  #figure out appropriate cutoff by solving for the r that gives p < 0.05 given n
  
  n = opls.model@descriptionMC[1,1] %>% as.numeric()
  t = qt(0.05, n)
  ...
  if(ncomp == 1){
    ...
    # if corr > 0.38, then its "important" variable
    mutate(detect_discr = ifelse(p1 > r, TRUE, FALSE),
           is_discr = ifelse(str_detect(Variable, "D"), TRUE, FALSE))
  } else if(ncomp > 1){
  
  #if there is more than 1 component, just use the first two.
    cor(scores[2:3], data) %>%
    t() %>%
    as_tibble(rownames = "Variable") %>% 
    rowwise(.) %>% 
...
```{r}
# opls.model <- pls.model <- control.pls[[3]]
...
control.k.plotdata <-
  ...
  mutate(Method = as.factor(Method) %>%
           fct_recode(PCA = "pca.k", `PLS-DA` = "pls.k", `PLS-DA VIP` = "pls.VIP.k"))
...
```{r}
panels <-
  plot_grid(control.p + theme(legend.position = "none")+ coord_cartesian(xlim = c(-0.7, 1)),
            needle.p + theme(legend.position = "none")+ coord_cartesian(xlim = c(-0.7, 1)),
            nrow = 1,
            ncol = 2,
            labels = "AUTO")
fig2 <-
