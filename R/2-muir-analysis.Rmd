...
```{r}
tom.pls.flipped <- 
  opls(select(tomato.filtered, precip, temp), as.matrix(X), fig.pdfC = "none", predI = 2)
get_modelinfo(tom.pls.flipped)
plot(tom.pls.flipped)
```
...
I feel like I read somewhere that RDA is like an improved CCA.  CCA was removed from Stéphane Dray's `ade4` package in 2017.

"Constrained correspondence analysis is indeed a constrained method: CCA does not try to display all variation in the data, but only the part that can be explained by the used constraints...CCA is a good choice if the user has clear and strong a priori hypotheses on constraints and is not interested in the major structure in the data set."

"According to folklore, rda should be used with “short gradients” rather than cca. However, this is not based on research which finds methods based on Euclidean metric [rda] as uniformly weaker than those based on Chi-squared metric [cca]."
...
```{r}
muir_cca <- cca(X ~ tomato.filtered$precip)
plot(muir_cca, scaling = "symmetric")
MVA.plot(muir_cca, "correlations")
```
Loadings:

```{r}
scores(muir_cca)$species
```
...
RsquareAdj(muir_cca)
```
~ 7% of variation explained by precipitation
...
```{r}
muir_rda <- rda(X~tomato.filtered$precip)
muir_rda
MVA.synt(muir_rda)
```
```{r}
plot(muir_rda, scaling = "symmetric")
biplot(muir_rda)
RVAideMemoire::MVA.plot(muir_rda)
```
...
RsquareAdj(muir_rda)
```
~ 5.8% of variation explained by precipitation

Significance test:

```{r}
RVAideMemoire::MVA.anova(muir_rda)
```


# CIA

Reviewer 1 recommended co-inertia analysis (CIA).

PCA-PCA CIA is analagous to RDA
...
Huh, this is significantly more confusing to use.  This is clearly a **two-step procedure**!

First, I have to do PCA on both X (plant x trait matrix) and Y (precipitation), then plug results from those into `coinertia()`

```{r}
dudi.X <- dudi.pca(df = X, nf = 100, scannf = FALSE)
dudi.Y <- dudi.pca(tomato.filtered$precip, nf = 1, scannf = FALSE)
```

```{r}
muir_cia <- coinertia(dudi.X, dudi.Y, scannf = FALSE, nf = 1)
summary(muir_cia)
```
```{r}
plot(muir_cia)
