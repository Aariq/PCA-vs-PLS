...
Combines scores and loadings.  To make a column-principal biplot, I think I need to standardize the scores on each axis to unit variance. (or SS = 1?? Or just scale loadings up by sqrt(n)??  Or maybe it just needs to look nice?)

```{r}
scaled_loadings <- get_loadings(tom.pca) %>% 
  mutate_if(is.numeric, ~.*sqrt(63))
...
ggplot(scaled_loadings) +
  ...
  geom_point(data = get_scores(tom.pca), aes(x = p1, y = p2, fill = tomato.filtered$precip), shape = 21, size = 2, inherit.aes = FALSE) +
  scale_fill_viridis_c() +
  labs(x = glue("PC1 ({tom.pca.data$axis_stats[1,1] * 100}%)"),
       y = glue("PC2 ({tom.pca.data$axis_stats[2,1] * 100}%)"),
       fill = "Annual Precipitation (mm)") +
