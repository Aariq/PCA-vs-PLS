...
Combines scores and loadings. I actually found a lot of **very confusing** and conflicting information on how to scale scores and loadings to make a bi-plot, so for now I've just multiplied the loadings by a factor that makes the plot look nice.

```{r}
scaled_loadings <- get_loadings(tom.pca) %>% 
  mutate_if(is.numeric, ~.*5)
scaled_scores <- get_scores(tom.pca) 
# prcomp(X, scale. = TRUE, rank. = 2) %>% biplot(scale = TRUE)
# biplot.princomp() just plots the scores and loadings on different axes--not a true biplot?

ggplot(scaled_loadings) +
  geom_point(data = scaled_scores, aes(x = p1, y = p2, fill = tomato.filtered$precip),
             shape = 21, size = 2, inherit.aes = FALSE) +
  geom_segment(aes(x = 0, y = 0, xend = p1, yend = p2),
               alpha = 0.6,
               arrow = arrow(length = unit(0.15, "cm"))) +
  geom_label_repel(aes(x = p1, y = p2, label = Variable),
