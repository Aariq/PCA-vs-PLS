...
Right now the way I'm simulating data might not be terribly realistic. There is one chunk of variables with no covariance (that's ok); another chunk covaries with eachother, but not with the grouping variable (also imaginable); the third chunk of variables also covary within each group, but not neccesariliy with the grouping variable.  Instead, I've just changed the means (intercepts?) of the variables for each group.

Maybe this is why PLS-DA is doing a poor job relative to PERMANOVA, and this data might not be terribly realistic (plot discriminating variables vs. group membership? vs one discriminating variable, color = group membership?).

Another possibility would be to create three chunks again±—--one that doesn't covary, one that covaries, and a third that covaries. Then take that third chunk, pick one of the variables and make it the Y variable. Then I could use that for regression PLS.  *Or* I could take that Y variable, `arrange(Y)`, and then assign groups such that rows with high values for Y are group A and those with low values of Y are group B.  Then all the other discriminating variables should co-vary with group membership and distinguish between groups becasue they were all set up to covary with Y.  This might be a more realistic dataset?  It might also be a situation where PLS-DA performs better than PERMANOVA.
...
sim.vcov <- function(p_noise,
  ...
  # make noise data
  if(p_noise > 0){
  S_noise <- matrix(rep(0.05, p_noise^2), ncol = p_noise)
  diag(S_noise) <- var
  set.seed(seed)
...
P = 30 #total number of variables
signal.prop = 0.25 #when there is signal, what proportion of variables are signal
cov_signal = 0.25 #covariance for signal variables
disc.prop = 0.5 #when there are discriminating variables, what proportion
