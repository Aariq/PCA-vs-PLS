...
- `p_noise`: how many variables with covariance = 0?
...
sim.vcov <- function(p_noise,
  ...
  # make noise data
  if(p_noise > 0){
  S_noise <- matrix(rep(0, p_noise^2), ncol = p_noise)
  diag(S_noise) <- 1
  set.seed(seed)
...
#calculated values
p_signal = round(N*signal.prop)
p_disc = round(N*disc.prop)
```
...
pca.data1.list <- map(sim.data1.list, ~opls(select(., -group), plotL = FALSE))
...
p1.testresults <- loadings.data1.list %>%
...
p2.testresults <- loadings.data1.list %>%
...
```{r include=FALSE}
pls.data1.list <- map(sim.data1.list, ~opls(select(., -group), grouping, predI = 2, permI = 200, plotL = FALSE))
```
```{r message=FALSE, warning=FALSE}
...
```{r}
set.seed(100)
sim.data2 <- sim.vcov(p_noise = N - p_signal,
                      p_signal = p_signal,
                      p_disc = 0,
...
Now, do PCA on all of them and get loadings.
```{r message=FALSE, warning=FALSE, include=FALSE}
pca.data2.list <- map(sim.data2.list, ~opls(select(., -group), plotL = FALSE))
...
p1.testresults <- loadings.data2.list %>%
  #maps t.test() function to all dataframes and converts output into dataframe with tidy()
  map_df(~t.test(.$p1~grouping) %>% tidy()) %>% 
  #selects just columns of interest
  select(PC1.effect.size = "estimate",
...
p2.testresults <- loadings.data2.list %>%
  map_df(~t.test(.$p2~grouping) %>% tidy()) %>%
  select(PC2.effect.size = "estimate",
         PC2.t = "statistic",
         PC2.p.value = "p.value")
...
Now do PLS-DA on all of them and get p-values (this will take a long time)
```{r include=FALSE}
pls.data2.list <- map(sim.data2.list, ~opls(select(., -group), grouping, predI = 2, permI = 200, plotL = FALSE))
```
```{r message=FALSE, warning=FALSE}
...
```{r}
set.seed(100)
sim.data3 <- sim.vcov(p_noise = N - p_disc,
                      p_signal = 0,
                      p_disc = p_disc,
                      cov_signal = cov_signal,
                      diff_disc = diff_disc,
...
Now, do PCA on all of them and get loadings.
```{r message=FALSE, warning=FALSE, include=FALSE}
pca.data3.list <- map(sim.data3.list, ~opls(select(., -group), plotL = FALSE))
...
p1.testresults <- loadings.data3.list %>%
  #maps t.test() function to all dataframes and converts output into dataframe with tidy()
  map_df(~t.test(.$p1~grouping) %>% tidy()) %>% 
  #selects just columns of interest
  select(PC1.effect.size = "estimate",
...
p2.testresults <- loadings.data3.list %>%
  map_df(~t.test(.$p2~grouping) %>% tidy()) %>%
  select(PC2.effect.size = "estimate",
         PC2.t = "statistic",
         PC2.p.value = "p.value")
...
Now do PLS-DA on all of them and get p-values (this will take a long time)
```{r include=FALSE}
pls.data3.list <- map(sim.data3.list, ~opls(select(., -group), grouping, predI = 2, permI = 200, plotL = FALSE))
```
```{r message=FALSE, warning=FALSE}
...
## Generate Data:
```{r}
sim.data4 <- sim.vcov(p_noise = N - p_signal - p_disc,
                      p_signal = p_signal,
                      p_disc = p_disc,
                      cov_signal = cov_signal,
                      diff_disc = diff_disc,
...
sim.data4.list <- map(1:nperm,
                      ~sim.vcov(p_noise = N - p_signal - p_disc,
                                p_signal = p_signal,
                                p_disc = p_disc,
                                cov_signal = cov_signal,
                                diff_disc = diff_disc,
...
Now, do PCA on all of them and get loadings.
```{r message=FALSE, warning=FALSE, include=FALSE}
pca.data4.list <- map(sim.data4.list, ~opls(select(., -group), plotL = FALSE))
...
p1.testresults <- loadings.data4.list %>%
  #maps t.test() function to all dataframes and converts output into dataframe with tidy()
  map_df(~t.test(.$p1~grouping) %>% tidy()) %>% 
  #selects just columns of interest
  select(PC1.effect.size = "estimate",
...
p2.testresults <- loadings.data4.list %>%
  map_df(~t.test(.$p2~grouping) %>% tidy()) %>%
  select(PC2.effect.size = "estimate",
         PC2.t = "statistic",
         PC2.p.value = "p.value")
...
Now do PLS-DA on all of them and get p-values (this will take a long time)
```{r include=FALSE}
pls.data4.list <- map(sim.data4.list, ~opls(select(., -group), grouping, predI = 2, permI = 200, plotL = FALSE))
```
```{r message=FALSE, warning=FALSE}
...
p.val_figure <- plot_grid(ggplot() + ggtitle("Distr. of p values from t-tests on PC1 (blue), PLS-DA (red), PERMANOVA (green)"), plots, ncol = 1, rel_heights = c(0.1, 1))
p.val_figure
