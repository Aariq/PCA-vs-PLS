...
```{r}
veen.mass <- read_excel(here("Veen et al", "Veen et al FE-data.xlsx"))
veen.env <-  read_excel(here("Veen et al", "Veen et al FE-data.xlsx"), sheet = 2)
veen.mass
veen.env <- veen.env %>% 
...
```{r}
plsdata <- veen %>%
  select(-ID, -transectID, -site, -mass4) %>%
...
plsr <- opls(select(plsdata, -mass12), plsdata$mass12, permI = 200)
