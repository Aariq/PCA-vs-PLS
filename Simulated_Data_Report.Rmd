...
- Switch PERMANOVA to RDA. PERMANOVA isn't a good comparison since it uses distance matrices.  Can't get loadings out.
...
4. PERMANOVA requires distance matrixes, so you can't get info about original variables from it.  It's not designed to work well with multicollinearity.
...
### 3a RDA
```{r}
test <- rda(sim3a[[1]] %>% select(-group) ~ group, data = sim3a[[1]])
View(test)
test2 <- MVA.synt(test)
test2[[1]]$tab$Proportion[1]
test3<-anova(test)
test3$`Pr(>F)`[1]
